...
export class UpdateService {
  ...
  private static promptForUpdate(version: AppVersion, isRequired: boolean) {
    const message = version.features?.length > 0
      ? `Version ${version.version} is available!\n\nWhat's new:\n${version.features.map(f => `• ${f}`).join('\n')}`
  ...
  static openStore() {
    ...
    ios: 'https://apps.apple.com/app/explorable/id[YOUR_APP_STORE_ID]', // You'll get this ID after App Store submission
...
export function UpdateChecker({ children }: { children: React.ReactNode }) {
  useEffect(() => {
    ...
    // Optional: Check periodically (every 4 hours)
