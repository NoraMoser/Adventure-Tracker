...
const ACHIEVEMENTS = [
  { id: 'first_activity', name: 'First Steps', icon: 'footsteps', requirement: 'Complete your first activity', color: '#4CAF50' },
  { id: 'first_location', name: 'Explorer', icon: 'location', requirement: 'Save your first location', color: '#2196F3' },
  { id: 'week_streak', name: 'Week Warrior', icon: 'flame', requirement: '7 day activity streak', color: '#FF9800' },
  { id: 'month_streak', name: 'Dedicated', icon: 'trophy', requirement: '30 day activity streak', color: '#FFD700' },
  { id: '10_activities', name: 'Active', icon: 'fitness', requirement: 'Complete 10 activities', color: '#9C27B0' },
  { id: '25_locations', name: 'Adventurer', icon: 'map', requirement: 'Save 25 locations', color: '#00BCD4' },
  { id: '100km', name: 'Century', icon: 'bicycle', requirement: 'Travel 100km total', color: '#8BC34A' },
  { id: '5_categories', name: 'Versatile', icon: 'apps', requirement: 'Try 5 different categories', color: '#FF5722' },
  { id: 'early_bird', name: 'Early Bird', icon: 'sunny', requirement: 'Activity before 7 AM', color: '#FFC107' },
  { id: 'night_owl', name: 'Night Owl', icon: 'moon', requirement: 'Activity after 9 PM', color: '#3F51B5' },
];
...
export default function StatisticsScreen() {
  ...
  const stats = useMemo(() => {
    ...
    const avgSpeed = activities.reduce((sum, act) => sum + act.averageSpeed, 0) / (totalActivities || 1);
    ...
    const personalRecords = {
      longestDistance: activities.reduce((max, act) => 
        act.distance > max.distance ? act : max, 
        { distance: 0, startTime: new Date() }
      ),
      longestDuration: activities.reduce((max, act) => 
        act.duration > max.duration ? act : max,
        { duration: 0, startTime: new Date() }
      ),
      fastestSpeed: activities.reduce((max, act) => 
        act.maxSpeed > max.maxSpeed ? act : max,
  ...
  const pieData = Object.entries(stats.activityTypes).map(([type, count], index) => ({
    name: type,
    population: count,
    color: [
      theme.colors.forest,
      ...
      <View style={styles.headerStats}>
        ...
        <View style={styles.statsGrid}>
          ...
          <View style={styles.smallStatCard}>
            ...
            <Text style={styles.smallStatNumber}>{formatDuration(stats.totalDuration)}</Text>
          ...
          <ProgressChart
            ...
            chartConfig={{
              ...
              color: (opacity: number = 1, index: number = 0) => {
                const colors = [theme.colors.forest, theme.colors.navy, theme.colors.burntOrange];
      ...
      <View style={styles.section}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          ...
          <PersonalRecordCard
            ...
            value={formatDuration(stats.personalRecords.longestDuration.duration)}
            unit=""
            icon="time"
            date={new Date(stats.personalRecords.longestDuration.startTime).toLocaleDateString()}
      ...
      <View style={styles.section}>
        ...
        <View style={styles.averageCards}>
          <View style={styles.averageCard}>
            ...
            <Text style={styles.averageValue}>{formatDistance(stats.avgDistance)}</Text>
            <Text style={styles.averageLabel}>per activity</Text>
          </View>
          <View style={styles.averageCard}>
            <Ionicons name="hourglass" size={24} color={theme.colors.navy} />
            <Text style={styles.averageValue}>{formatDuration(stats.avgDuration)}</Text>
            <Text style={styles.averageLabel}>per activity</Text>
          </View>
          <View style={styles.averageCard}>
