...
const AddWishlistModal = ({
  ...
  useEffect(() => {
    if (currentLocation && useCurrentLocation) {
      ...
      setSelectedLocation({ latitude: currentLocation.latitude, longitude: currentLocation.longitude });
    }
  }, [currentLocation, useCurrentLocation]);

  // Simple geocoding search (you can enhance this with Google Places API later)
  const handleLocationSearch = async () => {
    if (!locationSearch.trim()) return;
    
    setIsSearching(true);
    try {
      // For now, we'll use a simple approach with some popular destinations
      // In production, you'd use Google Places API or similar
      const popularPlaces: { [key: string]: { lat: number; lng: number } } = {
      ...
      const found = Object.entries(popularPlaces).find(([place]) => 
        place.includes(searchLower) || searchLower.includes(place)
      );

      if (found) {
        const [placeName, coords] = found;
        ...
        Alert.alert('Location Found', `Set to ${placeName.charAt(0).toUpperCase() + placeName.slice(1)}`);
      } else {
        Alert.alert(
          'Location Not Found',
          'Try searching for major cities or landmarks, or enter coordinates manually.\n\nFor more precise locations, you can look up coordinates on Google Maps and paste them here.',
          [
            { text: 'OK' },
            { 
  ...
  const handleSubmit = () => {
    ...
    if (!useCurrentLocation && !selectedLocation && (!latitude || !longitude)) {
      Alert.alert(
        'Location Required',
    ...
    const finalLat = useCurrentLocation && currentLocation 
      ? currentLocation.latitude 
      : (selectedLocation?.latitude || parseFloat(latitude));
    
    const finalLng = useCurrentLocation && currentLocation
      ? currentLocation.longitude
      : (selectedLocation?.longitude || parseFloat(longitude));
              ...
              {PRIORITY_LEVELS.map((level) => (
                <TouchableOpacity
                  ...
                  style={[
                    ...
                    priority === level.value && { backgroundColor: level.color + '20', borderColor: level.color }
                  ]}
                ...
                >
                  <Ionicons 
                    ...
                    size={20} 
                    color={priority === level.value ? level.color : theme.colors.gray} 
                  />
                  <Text style={[
                    styles.priorityText,
              ...
              <>
                <View style={styles.searchContainer}>
                  <TextInput
                    ...
                    placeholder="Search for a place (e.g., Paris, Machu Picchu)"
                    placeholderTextColor={theme.colors.lightGray}
                          ...
                          onPress: (coords) => {
                            if (coords) {
                              ...
                              if (parts.length === 2) {
                                ...
                                Alert.alert('Invalid Format', 'Please use format: latitude, longitude');
                              }
                            }
                          }
                        }
                ...
                >
                  <Ionicons name="keypad-outline" size={16} color={theme.colors.navy} />
                  <Text style={styles.manualCoordsText}>Enter coordinates manually</Text>
                </TouchableOpacity>

                {selectedLocation && (
                  <View style={styles.selectedLocationBox}>
                    <Ionicons name="checkmark-circle" size={16} color={theme.colors.forest} />
                ...
                <View style={styles.helpBox}>
                  <Ionicons name="information-circle-outline" size={16} color={theme.colors.lightGray} />
                  <Text style={styles.helpText}>
                    Tip: Search for famous landmarks, cities, or use &quot;Current Location&quot; for nearby places
                  </Text>
                </View>
              </>
            )}
...
export default function WishlistScreen() {
  ...
  const { wishlistItems, addWishlistItem, removeWishlistItem, convertToVisited } = useWishlist();
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            Alert.alert('Success', 'Location has been saved to your visited spots!');
          },
        },
      ]
    );
  ...
  const renderListItem = ({ item }: any) => {
    ...
    const priorityLevel = PRIORITY_LEVELS.find(p => p.value === item.priority) || PRIORITY_LEVELS[1];
    ...
    return (
      <View style={styles.listItem}>
          ...
          <View style={styles.listItemHeader}>
            ...
            <View style={styles.headerText}>
              ...
              <View style={styles.metaRow}>
                <View style={[styles.priorityBadge, { backgroundColor: priorityLevel.color + '20' }]}>
                  <Ionicons name={priorityLevel.icon as any} size={12} color={priorityLevel.color} />
                  <Text style={[styles.priorityBadgeText, { color: priorityLevel.color }]}>
                    {priorityLevel.label}
                  </Text>
                </View>
        ...
        <View style={styles.actionButtons}>
          ...
          >
            <Ionicons name="checkmark-circle" size={22} color={theme.colors.forest} />
          </TouchableOpacity>
          <TouchableOpacity
  ...
  const generateMapHTML = () => {
    ...
    const markers = filteredItems.map(item => {
      const category = categories[item.category as CategoryType] || categories.other;
      const priority = PRIORITY_LEVELS.find(p => p.value === item.priority) || PRIORITY_LEVELS[1];
  ...
  if (wishlistItems.length === 0 && !showAddModal) {
    return (
      <View style={styles.emptyContainer}>
        ...
        <Text style={styles.emptySubtitle}>Add places you dream of visiting!</Text>
        <TouchableOpacity 
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        {PRIORITY_LEVELS.map((level) => {
          const count = wishlistItems.filter(item => item.priority === level.value).length;
          return (
            <TouchableOpacity
              key={level.value}
              style={[
                styles.filterChip,
                selectedPriority === level.value && { backgroundColor: level.color, borderColor: level.color }
              ]}
      ...
      <View style={styles.viewToggle}>
        ...
        >
          <Ionicons name="list" size={20} color={viewMode === 'list' ? 'white' : theme.colors.forest} />
        ...
        >
          <Ionicons name="map" size={20} color={viewMode === 'map' ? 'white' : theme.colors.forest} />
        ...
        <View style={styles.mapContainer}>
          ...
          {showMapLegend && (
            <View style={styles.mapLegend}>
              ...
              <Text style={styles.legendText}>Ghost pins = Places to visit</Text>
